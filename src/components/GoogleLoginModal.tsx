...
import { LogIn, LogOut, CheckCircle2, Sparkles, ShieldCheck, Mail, ArrowRight, AlertTriangle, BookOpen } from 'lucide-react';
      ...
      <div className="w-full max-w-md bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl shadow-2xl overflow-hidden flex flex-col">
        ...
        <div className="p-6 bg-slate-900 text-white relative">
          <div className="flex items-center justify-between mb-3">
            ...
            <button
              onClick={onClose}
              className="text-slate-400 hover:text-white text-sm w-7 h-7 rounded-full bg-slate-800 flex items-center justify-center transition cursor-pointer"
              title="ปิด"
            >
              ✕
            </button>
        ...
        <div className="p-6 space-y-5">
          {currentUser ? (
            /* Logged In Returning User Welcome View */
            <div className="py-2 px-1 text-center space-y-5 animate-fadeIn">
              ...
              <div className="relative inline-block">
                ...
                />
                <div className="absolute bottom-0 right-0 bg-emerald-500 text-white p-1 rounded-full border-2 border-white dark:border-slate-900 shadow-xs">
              ...
              <div className="space-y-2">
                <div className="inline-flex items-center gap-1.5 px-3 py-1 bg-amber-500/15 border border-amber-500/30 rounded-full text-xs font-bold text-amber-700 dark:text-amber-300">
                  <Sparkles className="w-3.5 h-3.5 text-amber-500 shrink-0" />
                  <span>บัญชี Google Verified</span>
                </div>
              ...
              {/* Primary Action Button: Jump directly into Classroom */}
              <div className="pt-2 space-y-3">
                <button
                  ...
                  className="w-full py-4 px-5 bg-gradient-to-r from-amber-500 via-amber-600 to-yellow-500 hover:from-amber-600 hover:to-yellow-600 text-slate-950 font-extrabold text-sm rounded-2xl shadow-lg flex items-center justify-center gap-2.5 transition cursor-pointer active:scale-[0.98]"
                >
                ...
                <div className="flex items-center gap-2 pt-1 border-t border-slate-200 dark:border-slate-800">
                  <button
                    ...
                    className="flex-1 py-2 px-3 bg-slate-100 hover:bg-slate-200 dark:bg-slate-800 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-300 rounded-xl font-bold text-xs transition cursor-pointer"
                  >
                    {loadingFirebase ? 'กำลังเชื่อมต่อ...' : 'สลับบัญชี Google'}
                  </button>
                  {onLogout && (
                    <button
                      onClick={() => {
                      ...
                      className="flex-1 py-2 px-3 bg-rose-500/10 hover:bg-rose-500/20 text-rose-600 dark:text-rose-400 border border-rose-500/20 rounded-xl font-bold text-xs transition cursor-pointer"
                    >
                      ออกจากระบบ
                    </button>
                  )}
